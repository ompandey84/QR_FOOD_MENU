    ...
    return (
        <div className={`rounded-2xl border bg-gradient-to-br p-3 sm:p-5 ${colors[color]}`}>
            <div className="flex items-start justify-between mb-2 sm:mb-4">
                ...
                {trend !== undefined && (
                    <span className={`text-xs font-bold px-2 py-1 rounded-lg ${trend >= 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'}`}>
                        {trend >= 0 ? '+' : ''}{trend}%
                    </span>
                )}
            </div>
...
export default function AnalyticsPage() {
    ...
    const loadMetrics = useCallback(async (restId) => {
        ...
        // Build top dishes from order_items
        const dishMap = {};
        valid.forEach(order => {
        ...
        // Build real notifications from recent pending orders
                ...
                <main className="p-4 lg:p-8 pb-32 lg:pb-8 space-y-6 lg:space-y-8">
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            {/* Recent Orders */}
                            <div className="lg:col-span-2 bg-white rounded-2xl border border-[#F4F2E6] p-6 shadow-sm">
                            ...
                            {/* Top Dishes */}
                            <div className="bg-white rounded-2xl border border-[#F4F2E6] p-6 shadow-sm">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-lg font-black text-charcoal">Top Dishes</h3>
                                </div>
                                {topDishes.length === 0 ? (
                                    <div className="h-48 flex flex-col items-center justify-center text-slate-400">
                                        <MdTableRestaurant className="w-10 h-10 mb-2 opacity-30" />
