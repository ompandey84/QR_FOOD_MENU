...
export default function QRCodesPage() {
    ...
    const [restaurantName, setRestaurantName] = useState('SmartMenu');

    const { limits, canAddQR } = useSubscription(restaurantId);

    useEffect(() => {
        async function loadData() {
            const { data: { user } } = await supabase.auth.getUser();
            if (!user) return navigate('/login');
            const { data: rest } = await supabase.from('restaurants').select('id, plan, name').eq('owner_id', user.id).single();
            if (!rest) return;
            ...
            if (savedCount) {
                ...
                // Default table count logic based on plan if no saved count
                const planCounts = { starter: 3, growth: 5, pro: 12 };
                setTableCount(planCounts[rest.plan] || 3);
            }
    ...
    const handleUpdateTableCount = (newCount) => {
        ...
        if (count > limits.maxQrs && count > visibleTables.length) {
            setErrorDialog({
                isOpen: true,
                title: 'Limit Reached',
                message: `Your current plan only allows up to ${limits.maxQrs} QR codes. Please upgrade your plan to generate more.`
            });
            return;
        }
        
        // If they enter a count less than the visible tables, we don't automatically delete. We just cap tableCount.
        // Actually, replacing visibleTables requires matching `tableCount` with what's visible.
        setTableCount(count);
        localStorage.setItem(`tableCount_${restaurantId}`, count);
    };

    const handleAddTable = () => {
        if (!canAddQR(visibleTables.length)) {
            setErrorDialog({
                isOpen: true,
                title: 'Limit Reached',
                message: `Your current plan only allows up to ${limits.maxQrs} QR codes. Please upgrade your plan to generate more.`
                ...
                <main className="p-6 lg:p-8">
                    {loading ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {[1, 2, 3, 4, 5, 6].map(i => (
                                <div key={i} className="bg-white p-6 rounded-2xl border border-[#F4F2E6] animate-pulse flex flex-col items-center">
                                    <div className="w-full flex justify-between items-center mb-4">
                                        <div className="h-3 w-12 bg-slate-200 rounded" />
                                        <div className="h-6 w-16 bg-slate-200 rounded-lg" />
                                    </div>
                                    <div className="w-[150px] h-[150px] bg-slate-100 rounded-xl mb-4" />
