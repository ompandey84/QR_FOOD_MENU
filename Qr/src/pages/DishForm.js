...
export default function DishForm() {
    ...
    function handleImageChange(e) {
        ...
        if (file.size > 5 * 1024 * 1024) {
            setErrorDialog({ isOpen: true, title: 'File Too Large', message: 'Image size must be less than 5MB.' });
            return;
        }
        setImageFile(file);
    ...
    async function uploadImage(file) {
        try {
            const compressed = await compressImage(file, { maxWidthOrHeight: 1200, quality: 0.8 });
            const ext = 'jpg';
            const fileName = `dishes/${Date.now()}-${Math.random().toString(36).substr(2, 9)}.${ext}`;
            const { error: uploadErr } = await supabase.storage
                .from('menu-images').upload(fileName, compressed, { cacheControl: '3600', upsert: false });
        ...
        <div className="min-h-screen bg-background-light">
            ...
            <main className="max-w-[1200px] mx-auto w-full p-6 lg:p-10">
                ...
                <div className="flex items-center gap-2 mb-6 text-sm">
                    <button onClick={() => navigate('/menu-manager')} className="text-primary font-medium hover:underline flex items-center gap-1">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="hidden sm:flex items-center gap-3">
                        <button onClick={() => navigate('/menu-manager')} className="btn-secondary text-sm !py-2.5">Discard</button>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        ...
                        <div className="lg:col-span-8 flex flex-col gap-6">
                            ...
                            <div className="flex sm:hidden items-center gap-3">
                                <button type="button" onClick={() => navigate('/menu-manager')} className="flex-1 btn-secondary">Discard Changes</button>
