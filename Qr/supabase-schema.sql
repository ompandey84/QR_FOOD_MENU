...
ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS plan TEXT DEFAULT NULL;
ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS whatsapp_receipts_enabled BOOLEAN DEFAULT false;
ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS checkout_fields JSONB DEFAULT '{"name": true, "phone": true, "table": false}'::jsonb;

-- Add instructions column to orders table
ALTER TABLE orders ADD COLUMN IF NOT EXISTS instructions TEXT DEFAULT NULL;

-- Add is_featured column to menu_items for star dish feature
ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS is_featured BOOLEAN DEFAULT false;

-- Enable Realtime for orders table so live updates work in the Admin Dashboard
BEGIN;
